...
export async function POST(request: NextRequest) {
  try {
    ...
    if (paymentProvider === 'IOTEC_MTN' || paymentProvider === 'IOTEC_AIRTEL') {
      // Use verifyIotecPayment for quick status check (single request)
      // The frontend will handle polling by calling this endpoint multiple times
      const result = await verifyIotecPayment(transactionId)
